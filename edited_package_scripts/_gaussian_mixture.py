...
def _estimate_gaussian_covariances_full(resp, X, nk, means, reg_covar):
    ...
    # Compute the difference between each sample and each mean
    diff = X[:, np.newaxis, :] - means[np.newaxis, :, :]  # Shape: (n_samples, n_components, n_features)

    # Compute weighted outer products for all components at once
    weighted_outer = resp[:, :, np.newaxis, np.newaxis] * (
        diff[:, :, :, np.newaxis] * diff[:, :, np.newaxis, :]
    )  # Shape: (n_samples, n_components, n_features, n_features)

    # Sum over all samples to compute covariance matrices
    covariances_ = weighted_outer.sum(axis=0) / nk[:, np.newaxis, np.newaxis]

    # Add regularization to the diagonal
    covariances_[:, np.arange(n_features), np.arange(n_features)] += reg_covar
...
def _compute_precision_cholesky(covariances, covariance_type):
    ...
    if covariance_type == "full":
        ...
        try:
            # Perform Cholesky decomposition for all components
            cov_chol = np.linalg.cholesky(covariances)  # Shape: (n_components, n_features, n_features)

            # Compute the lower triangular inverse for each component
            precisions_chol = np.linalg.solve(cov_chol, np.eye(covariances.shape[1])[np.newaxis, :, :]).transpose(0, 2, 1)
...
def _estimate_log_gaussian_prob_numba(X, means, precisions_chol, covariance_type, log_det, n_samples, n_features, n_components):
    ...
    if covariance_type == "full":
        ...
        means_expanded = means[np.newaxis, :, :]  # Shape: (1, n_components, n_features)

        # Compute the difference between X and means
        diff = X_expanded - means_expanded  # Shape: (n_samples, n_components, n_features)

        # Perform batch matrix multiplication: diff @ precisions_chol
        y = np.einsum('ncp,cpf->ncf', diff, precisions_chol)  
        # Shape: (n_samples, n_components, n_features)

        # Compute squared sum along the last axis
        log_prob = np.sum(y**2, axis=2)
...
def _estimate_log_gaussian_prob(X, means, precisions_chol, covariance_type):
    ...
    return _estimate_log_gaussian_prob_numba(X, means, precisions_chol, covariance_type, log_det, n_samples, n_features, n_components)
